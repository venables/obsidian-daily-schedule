...
import { buildMeetingNotePath, buildMeetingNoteContent, createOrOpenMeetingNote } from "./meeting";
...
export class ScheduleView extends ItemView {
  ...
  async refresh(): Promise<void> {
    ...
    if (settings.calendars.length === 0) {
      this.renderEmpty("No calendars configured. Open plugin settings to add one.");
      return;
    }
    ...
    try {
      this.events = await fetchCalendarEvents(settings.calendars, settings.ignorePatterns);
  ...
  private renderEmpty(message: string): void {
    ...
    if (message.includes("settings")) {
      ...
      link.addEventListener("click", (e) => {
        ...
        (this.app as any).setting.openTabById("daily-schedule");
      });
    }
  }
  ...
  private renderError(): void {
    ...
    const retryBtn = errorEl.createEl("button", { text: "Retry", cls: "ds-retry-btn" });
    ...
    for (const event of this.events) {
      const isPast =
        !event.allDay && event.end
          ...
          : !event.allDay && event.start.getTime() < now.getTime() - 60 * 60 * 1000;

      const isCurrent =
        !event.allDay && event.end
          ? event.start.getTime() <= now.getTime() && event.end.getTime() > now.getTime()
          : false;
      ...
      if (event.attendees.length > 0) {
        const emailMap = buildEmailMap(this.app, [...this.plugin.settings.peopleFolders]);
        const resolved = resolveAttendees(event.attendees, emailMap, this.plugin.settings.myEmails);
  ...
  private async handleEventClick(event: ScheduleEvent): Promise<void> {
    ...
    const attendees = resolveAttendees(event.attendees, emailMap, settings.myEmails);
