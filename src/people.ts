...
export function buildEmailMap(app: App, peopleFolders: readonly string[]): EmailMap {
...
function extractEmails(frontmatter: Record<string, unknown>): readonly string[] {
...
export function resolveAttendees(
  ...
  const myEmailSet = new Set(myEmails.map((e) => e.toLowerCase().trim()).filter(Boolean));
  const seen = new Set<string>();

  return attendees
    .filter((a) => {
      const email = a.email.toLowerCase().trim();
      if (myEmailSet.has(email)) return false;
