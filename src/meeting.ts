...
import { cleanTitle, ensureFolderExists, formatDate, meetingNotePath } from "./helpers";

export function buildMeetingNotePath(basePath: string, event: ScheduleEvent): string {
...
export function buildMeetingNoteContent(
  ...
  const lines = ["---", "type: meeting", `date: ${dateStr}`, `title: "${title}"`];
...
export async function createOrOpenMeetingNote(
  ...
  if (existing) {
    ...
    new Notice("Opened existing meeting note");
    return;
  }
